...
- **`SafeERC20` desde el principio**: `transfer`/`transferFrom` siempre devolvieron `bool`, pero algunos tokens (USDT, BNB) NO revierten en fallo — devuelven `false`. Si no chequeás el bool, el bug es silencioso. `SafeERC20` te lo resuelve.
